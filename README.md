...
```bash
node benchmarks/run.mjs          # 19 个真实任务：修 bug / 实现 / 重构 / 写测试 / 写脚本 / 约束遵守 / 性能优化
node benchmarks/selfcheck.mjs    # 用例自检：初始态必须失败 / 参考修复必须通过 / 作弊必须被拒
```
...
**误导性失败信息**（18-decoy-root-cause）、**跨模块重构 + 隐藏约束**（19-shared-pagination）。

### 评测结论（诚实版）

- **用例自检 19/19 通过**（2026-09-11 实测）：每个用例都验过"初始必失败、正确解必通过、作弊被拒"。
  其中 18/19 两个用例专门设计成**能区分"真修"与"浅层修法"**：只把可见测试改绿（改测试、就地打补丁、
  保留 4 份重复实现）会被隐藏约束抓住，实测均被拒绝。
- **难度已饱和**：抽测 8 个用例（含最难的 5-bug 多模块题与共享根因题）分别交给新旧两版 Claude Code
  执行器（2.1.88 / 2.1.267），**两边全部 PASS**；新增的 18/19 同样两边全过 —— 因此"通过率"不再有区分度，
  **成本曲线才是能力信号**（同一道 5-bug/4-模块题：10 轮 vs 21 轮、缓存读 12.5 万 vs 22 万）。
- **端到端全量跑分对上游稳定性敏感**：曾出现前两个用例连续 524（上游超时）导致中断，单用例重跑均通过。
  这是执行器/网络问题，不是能力问题——报告里区分记录，不混为一谈。
