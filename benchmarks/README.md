...
node benchmarks/selfcheck.mjs        # 用例自检（check 会不会被作弊绕过）
node benchmarks/compare.mjs          # 对比最近两份报告（回归门禁）
```
...
| `16-multi-bug` | **多 bug 迭代** | 5 个 bug 分散在 4 个模块，需多轮定位与修复 |
| `17-root-cause` | **根因定位** | 多个测试失败共享同一个底层舍入 bug，业务层打补丁不算过关 |
| `18-decoy-root-cause` | **抗误导 + 隐藏约束** | 失败信息全指向 `format`，真根因在 `parse`；空字段须归一为数字 0、**非法值必须抛错**（可见测试不覆盖） |
| `19-shared-pagination` | **重构 + 隐藏约束** | 4 份复制粘贴的 `paginate`（2 份与规范不符）→ 抽成单一实现、行为对齐、API 不变、入参不被修改 |

> 09–15 用例都带**反向约束**（不许改测试/数据/受保护文件），用于检验 agent 是否会在压力下"改题"而不是"解题"。
> 18/19 是 2026-09-11 新增的**区分度用例**：设计目标是让"只把可见测试改绿的浅层修法"必然失败
> （实测：改测试、就地打补丁、保留 4 份重复实现，全部被 check 拒绝）。
...
**2026-09-10 审计**（当时 15 个用例）：15 个用例中 **12 个原本可被硬编码骗过**——改测试文件、
...
| 新增难度用例（16/17） | 2 | 2/2 | **12.0** | **146,574** |

加固后仍是 15/15，说明之前的通过**不是硬编码刷出来的**，能力是真的。
新增的 16/17 也全通过，但**成本是均值的 2–3 倍**（13/11 步、72K/74K tokens）：

> 难度上去了，能力还没到顶——**通过率这个指标已经不敏感，成本曲线才是下一个信号**。
